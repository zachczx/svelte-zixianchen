...
The blog uses a custom inline form immediately after each listed article. The [Blog article feedback survey](https://us.posthog.com/project/604259/surveys/01a08f68-1080-0000-b817-2c3f4e1b3d1a) must be launched with **API** presentation. Its supported schema is one three-choice question followed by one optional open-text question. Questions and choices are loaded from PostHog; changing the schema requires updating the form. Stopped, missing, unlaunched, or incompatible surveys stay hidden. The blog owns article eligibility; PostHog's popup delays and global display frequency are not used.

The form asks whether the article was relevant, offers an optional comment, and requires an explicit send. It inherits the blog's dark/light theme. A successful response is remembered per canonical article path in this browser; reading another article remains eligible immediately. No popup or cooldown is needed. Clearing storage or using another browser allows another response; this is a courtesy guard, not an authenticated voting system.

Submissions use PostHog's public capture endpoint through the managed proxy and the SDK's anonymous identity. The form waits for an accepted response, then shows thanks. Failed requests retain the response and offer retry with the same event UUID and payload. No answer text is stored locally. Responses carry `article_path`, `article_title`, the canonical URL, stable question IDs, and a submission ID. Break down `survey sent` by `article_path` to review individual articles. Survey impressions are captured only when the inline form enters the viewport. Automatic PostHog survey rendering is disabled to prevent duplicate popups.
...
For manual testing, set `PUBLIC_POSTHOG_TEST_MODE=true` and `PUBLIC_POSTHOG_TEST_KEY` to a **separate test project's public token**, then run `pnpm dev` on localhost. Create an API survey with the supported schema in the test project and update the development survey ID in `article-feedback.ts` to match. The test key must differ from `PUBLIC_POSTHOG_KEY`; production builds and remote preview hosts cannot use this bypass. Restart the dev server after environment changes.

Run `pnpm exec playwright install chromium` once, then `pnpm test:feedback` for repeatable browser checks. The tests use the installed PostHog SDK and custom survey UI with isolated network fixtures, verifying proxy paths, answers in serialized submission events, article-specific completion, repeat eligibility on other articles, retry, launch state, responsive display and theme switching, and blocked-request behavior. They do not send fake responses to PostHog. A separate local Astro server and temporary caches/results run under `.tmp/feedback-tests`, cleaned before and after each run. These tests verify the integration; a deployed first-event check is still needed to verify the hosting build variables and account settings.
...
After deployment, verify a pageview reaches the correct project and preview a targeted survey on the production domain. Check display and submission, including a narrow mobile viewport. Blocked PostHog requests should leave navigation and the contact form usable. Localhost is intentionally excluded, even when previewing a production build.
